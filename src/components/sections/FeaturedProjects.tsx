...
const FeaturedProjects = () => {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    // Only update position if we're inside the component bounds roughly
    setMousePos({ x: e.clientX, y: e.clientY });
  };

  return (
    <section className="py-32 border-b border-white/10 relative" onMouseMove={handleMouseMove}>
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <h3 className="text-4xl md:text-6xl font-syne font-bold uppercase tracking-tighter">
            Selected Projects
          </h3>
        </motion.div>
          ...
          {projects.map((project, index) => (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-baseline gap-4 md:gap-12 z-10 relative pointer-events-none">
                ...
                <h4 className="text-4xl md:text-5xl font-syne font-bold uppercase transition-transform duration-500 group-hover:translate-x-4">
                  {project.title}
                </h4>
              </div>
        ...
        <div className="mt-16 flex justify-center">
          <Link to="/projects" className="border border-white px-8 py-4 uppercase tracking-widest text-sm hover:bg-white hover:text-black transition-colors hover-target">
            View All Work
          </Link>
        </div>
      </div>
      ...
      <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-0 hidden md:block" style={{ pointerEvents: 'none' }}>
